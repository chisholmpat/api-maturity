...
exports.updateClient = function(client, res, callback) {
    ...
    knex('client').select('email').where('Client.id', client.id).asCallback(function(err, rows) {
        console.log(err);
        console.log(rows);
        if (rows) {
            oldEmail = rows[0].email;
            console.log("Old Email: ", oldEmail);
            knex('userclients').update({
                    email: client.email
                })
                .where('email', oldEmail.toUpperCase()).asCallback(function(err, rows) {
                    console.log(err);
                    console.log(rows);
                });
